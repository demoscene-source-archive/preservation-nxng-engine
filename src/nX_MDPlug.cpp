	/*----------------------------------------------------------------------------
		...
		nXng Entity Displacement plugin MDPlug implementation file
		This module enables high realism physic collision-deformations in nX.
...
bool	nX_Entity::LoadMDD(char *file)
{
	...
	wsprintf(str, "    nX_Entity::LoadMDD(\"%s\");", file);
	pnX->Verbose(str);
	obuffer = buffer;
	...
	if (!size)
	{
		pnX->Verbose("        ERROR: Loading MDD file... MDPlug disabled,");		
		return false;
	}

	// MDD 1.0~2.0 File LOADER by Emmanuel JULIEN 2000
	// Motion Designer is Copyright INO. 1998
	// -----------------------------------------------
